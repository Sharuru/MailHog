...
func (maildir *Maildir) List(start, limit int) (*data.Messages, error) {
	...
	files, _ := ioutil.ReadDir(maildir.Path)

	if start+limit > len(files) {
		limit = len(files) - start
	}

	start = len(files) - start - 1
	end := start - limit
	...
	for i := start; i > end; i-- {
		b, err := ioutil.ReadFile(filepath.Join(maildir.Path, files[i].Name()))
		if err != nil {
			return nil, err
		}
		msg := data.FromBytes(b)
		// FIXME domain
		m := *msg.Parse("MailTrap.Internal")
		m.ID = data.MessageID(files[i].Name())
		m.Created = files[i].ModTime()
...
func (maildir *Maildir) Load(id string) (*data.Message, error) {
	...
	// FIXME domain
	m := data.FromBytes(b).Parse("MailTrap.Internal")
